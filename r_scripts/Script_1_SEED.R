...
# Get contingency table of diseases from gwas studies
dis=table(temp[,"disease"])

# Filter to >=2 diseases associated to gene
dis=dis[dis>1]
...
temp=astro(node.gwas,as.data.frame(string),all.nodes=T)

t2<-induced_subgraph(temp, 1:10)
